...
class WOO_Cover_Fee_Updater {

	const REPO = 'biscuitstudios/woo-cover-processing-fee';

	const CACHE_KEY = 'woo_cover_fee_updater_release';
	...
	private function fetch_release() {
		...
		if ( is_wp_error( $response ) ) {
			error_log( '[Woo Cover Fee] Update check failed: ' . $response->get_error_message() );
		...
		if ( 200 !== $code ) {
			...
			error_log( '[Woo Cover Fee] Update check failed: HTTP ' . $code . ' from ' . self::REPO );
		...
		if ( ! is_array( $body ) || empty( $body['tag_name'] ) ) {
			error_log( '[Woo Cover Fee] Update check failed: no tag_name in the response for ' . self::REPO );
		...
		if ( ! preg_match( '/^\d+\.\d+\.\d+/', $version ) ) {
			error_log( '[Woo Cover Fee] Update check failed: unusable tag "' . $body['tag_name'] . '"' );
		...
		if ( '' === $package ) {
			error_log( '[Woo Cover Fee] Update check failed: release ' . $body['tag_name'] . ' has no zip asset.' );
	...
	private function render_changelog( $release ) {
		...
		if ( '' === $body ) {
			$out .= '<p>' . esc_html__( 'No release notes were published for this version.', 'woo-cover-processing-fee' ) . '</p>';
		} else {
		...
		if ( '' !== $release['url'] ) {
			$out .= sprintf(
				...
				esc_url( $release['url'] ),
				esc_html__( 'View this release on GitHub', 'woo-cover-processing-fee' )
			);
		}
